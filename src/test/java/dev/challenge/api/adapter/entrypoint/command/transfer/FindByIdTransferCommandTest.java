...
class FindByIdTransferCommandTest {
  ...
  @InjectMocks
  private FindByIdTransferCommand findByIdTransferCommand;
  ...
  void testExecute() {
    ...
    expectedTransferModel.setId(transferId);

    TransferDto expectedTransferDto = new TransferDto();
    expectedTransferDto.setId(transferId);

    when(transferService.findById(transferId)).thenReturn(expectedTransferModel);
    when(customMapper.map(expectedTransferModel, TransferDto.class)).thenReturn(expectedTransferDto);

    // Act
    TransferDto result = findByIdTransferCommand.execute(transferId);
